...
function getPolynom(...rest) {
  let powerLength = rest.length;
  const poly = [];
  rest.forEach((element) => {
    powerLength -= 1;
    poly.push({ el: element, power: powerLength });
  });

  return function some(x) {
    return poly.length === 0
      ? null
      : poly.reduce((a, b) => a + b.el * x ** b.power, 0);
  };
}
...
function logger(func, logFunc) {
  return function some(...rest) {
    logFunc(
      `${func.name}(${rest.map((el) => JSON.stringify(el)).join(',')}) starts`
    );
    const result = func(...rest);
    logFunc(
      `${func.name}(${rest.map((el) => JSON.stringify(el)).join(',')}) ends`
    );
    return result;
  };
}
...
function partialUsingArguments(fn, ...args1) {
  return function some(...args2) {
    if (args2 === undefined) return fn(...args1);
